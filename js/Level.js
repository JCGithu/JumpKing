class Level{
  ...
  showChat(){
    push();
    image(this.levelImage,width/2,0);
    pop();
  }
}
...
function addChat(temp){
  levels.push(temp);
  let tempCopy = new Level();
  temp.lines.forEach((v)=> {
    tempCopy.lines.push(new Line(v.x1 + (width/2), v.y1, v.x2 + (width/2), v.y2));
  })
  chatLevels.push(tempCopy);
}
...
async function setupLevels() {
  ...
  for (let i = 0; i < chatLevels.length; i++) {
    chatLevels[i].levelImage = levelImages[i];
    if ((i >= 25 && i <= 31)) {
      chatLevels[i].isBlizzardLevel = true;
    }
    if ((i >= 36 && i <= 38)) {
